...
- **📷 カメラキーフレーム（v2.4）** — カット内に任意個のカメラKFを打ち、タイムライン上の◇をドラッグして時刻を編集。「2秒静止→1秒で寄る→0.5秒でパン」のような複合ワークが、再生・動画書き出し・絵コンテ・AIプロンプトの全系統に反映される。カットを分割しても境界にKFが挿入されて動きが途切れない
- **🎯 フレーミング対象の指定（v2.4）** — カメラごとに **フリー（手動）／対象キャラ** を選択。フリーならキャラの有無に関わらず完全手動で操作でき、対象を選ぶとサイズ合わせ（EWS〜ECU/OTS/POV/2S）が有効になる
...
APIキー（いずれも任意）は起動後の画面から入力するか、`.env.example` を `.env` にコピーして設定します。

| キー | 用途 | 環境変数 |
|---|---|---|
| ElevenLabs | TTS＋リップシンク | `VITE_ELEVENLABS_API_KEY` |
| Anthropic | シーンチャット（日本語でシーン操作） | `VITE_ANTHROPIC_API_KEY` |

優先順位は「画面からの手入力（localStorage）→ `.env`」。
⚠️ `VITE_` の値はビルド時にJSバンドルへ平文で焼き込まれます。キーを入れてビルドした `dist/` は公開しないでください。
